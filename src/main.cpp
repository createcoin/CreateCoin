...
const string strMessageMagic = "CreateCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // CreateCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // CreateCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight >= 1 && pindexBest->nHeight <= 500)
      {
        nSubsidy = 25 * COIN;
...
// CreateCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes CreateCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // CreateCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // CreateCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // CreateCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // CreateCoin: track money supply and mint amount info
...
// CreateCoin: total coin age spent in transaction, in the unit of coin-days.
...
// CreateCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // CreateCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // CreateCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // CreateCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // CreateCoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 110; //coinbase maturity change to 180 blocks
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // CreateCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // CreateCoin: check proof-of-stake
    ...
    // CreateCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // CreateCoin: getblocks may not obtain the ancestor block rejected
    ...
    // CreateCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CreateCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "FitCoin is X13 POW-POS";
        CTransaction txNew;
        txNew.nTime = 1402492363;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402492363;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6317987;
        if(fTestNet)
        {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("28c1a8397ae1d59d9f5523f986f55c8d8d8428c765e3e935aaca487546ae82cf"));
        block.print();
        ...
        // CreateCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x71, 0x45, 0x13, 0x05 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // CreateCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // CreateCoin: tell downloading node about the latest block if it's
